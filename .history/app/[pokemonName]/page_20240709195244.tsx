...
import { getPokemon, PokemonList } from '@/libs/pokemonApi'
...
export default async function PokemonPage({ params }: PokemonDetailProps) {
  ...
  const pokemonObject = await PokemonList(pokemonName)

  const speciesResponse = await fetch(`https://pokeapi.co/api/v2/pokemon-species/${pokemonObject.name}/`);
