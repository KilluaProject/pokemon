                ...
                <div className="flex flex-col gap-3">
                        {typeData.map((type, index) => (
                            <div key={index} className="p-2 border rounded-md">
                                <h3 className="text-xl">{type.name.charAt(0).toUpperCase() + type.name.slice(1)}</h3>
                                <p>Generation: {type.generation?.name.replace('-', ' ') || 'Unknown'}</p>
                                <p>
                                    Double Damage From: {type.damage_relations?.double_damage_from?.map((d: any) => d.name).join(', ') || 'None'}
                                </p>
                            </div>
                        ))}
                    </div>
